...
Cross-platform desktop app for contributing GPU or CPU compute to the [Gridlock](https://grid-lock.tech) inference network. Built with **Electron** and a bundled **Python** worker daemon that connects to the [router](https://github.com/Gridlockcompute/router) on Robinhood Chain and runs local inference via **Ollama**.
...
4. Enter your `0x` wallet address in Settings
...
Production router URL defaults to `https://api.grid-lock.tech`. Override for local development:
...
| `GRIDLOCK_ATTESTATION_QUOTE_FILE` | — | Production NRAS/SEV quote JSON for TEE registration |
| `GRIDLOCK_ATTESTATION_QUOTE_JSON` | — | Inline attestation quote JSON |
...
```bash
GRIDLOCK_BACKEND_URL=http://127.0.0.1:8080 npm run dev
```
