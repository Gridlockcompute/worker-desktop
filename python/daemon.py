...
def register_with_backend():
    ...
    if not addr:
        ...
        state["last_backend_error"] = "Invalid EVM wallet address (expected 0x…)"
        log({"event": "register_skipped", "msg": "valid 0x wallet required"})
        return
    ...
    quote_payload = evm_wallet.resolve_registration_attestation_quote(
        BACKEND_URL, addr, TEE_CAPABLE, _SSL_CTX,
    )
    if quote_payload:
...
def ensure_ws_loop():
    """Start or restart the WebSocket loop so the router can dispatch jobs."""
    global _ws_thread
    if not state["running"]:
        return
    if _ws_thread and _ws_thread.is_alive() and _active_ws is not None:
        return
    if _ws_thread and _ws_thread.is_alive():
        stop_ws_connection()
        time.sleep(0.2)
    _ws_thread = threading.Thread(target=ws_loop, daemon=True, name="gridlock-ws")
    _ws_thread.start()
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if self.path == "/worker/start":
            wallet = evm_wallet.normalize_evm_address(state["worker_address"] or "")
            if not wallet:
                self._json({"ok": False, "error": "wallet_required", "message": "Enter a valid 0x EVM wallet before starting."}, 400)
                return
            state["worker_address"] = wallet
            ...
            if not state["running"]:
                ...
                if _worker_thread is None or not _worker_thread.is_alive():
                    _worker_thread = threading.Thread(target=worker_loop, daemon=True, name="gridlock-worker")
            ...
            self._json({"ok": True, "backend_ok": state["backend_ok"], "inference_backend": state["inference_backend"],
                        "effective_compute": state["effective_compute"], "ws_connected": _active_ws is not None,
                        "backend_url": BACKEND_URL})
            ...
            body = self._read_json()
            addr = evm_wallet.normalize_evm_address(str(body.get("address", "")))
            if not addr:
